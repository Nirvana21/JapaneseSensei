  ...
  return (
    <div className="min-h-screen bg-[#100c08] text-[#f5ede0]">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        {/* Controls */}
        <div className="bg-white/[0.04] rounded-2xl border border-white/[0.08] p-4 sm:p-6 mb-6 animate-fade-in-up">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-[#f5ede0]/60 mb-1">Forme</label>
              <select value={formKind} onChange={e => setFormKind(e.target.value as AdjFormKind)} className="w-full px-3 py-2 rounded-lg border border-white/[0.10] bg-white/[0.06] text-[#f5ede0]">
                {FORM_OPTIONS.map(o => (
            ...
            {needsPoliteness && (
              <div>
                <label className="block text-sm text-[#f5ede0]/60 mb-1">Politesse</label>
                <select value={politeness} onChange={e => setPoliteness(e.target.value as Politeness)} className="w-full px-3 py-2 rounded-lg border border-white/[0.10] bg-white/[0.06] text-[#f5ede0]">
            ...
            {needsTense && (
              <div>
                <label className="block text-sm text-[#f5ede0]/60 mb-1">Temps</label>
                <select value={tense} onChange={e => setTense(e.target.value as Tense)} className="w-full px-3 py-2 rounded-lg border border-white/[0.10] bg-white/[0.06] text-[#f5ede0]">
            ...
            {needsPolarity && (
              <div>
                <label className="block text-sm text-[#f5ede0]/60 mb-1">Polarité</label>
                <select value={polarity} onChange={e => setPolarity(e.target.value as Polarity)} className="w-full px-3 py-2 rounded-lg border border-white/[0.10] bg-white/[0.06] text-[#f5ede0]">
        ...
        {current && (
          <div className="bg-white/[0.04] rounded-3xl border border-white/[0.08] p-6 animate-fade-in-up">
            <div className="text-center mb-4">
              <p className="text-sm text-[#f5ede0]/50">Adjectif</p>
              <div className="text-4xl font-bold text-[#c9a84c]">{current.dict}</div>
              <div className="text-sm text-[#f5ede0]/50">{current.reading} • {current.meaningFr}</div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-stretch">
              ...
              <button onClick={shuffleAdj} className="px-4 py-3 rounded-xl bg-white/[0.06] hover:bg-white/[0.10] border border-white/[0.10] text-[#f5ede0] font-semibold">Nouvel adjectif</button>
            </div>
            ...
            <div className="mt-4">
              <button
                ...
                className="px-3 py-2 text-sm rounded-lg bg-white/[0.06] hover:bg-white/[0.10] text-[#f5ede0]/70 border border-white/[0.10]"
              >
