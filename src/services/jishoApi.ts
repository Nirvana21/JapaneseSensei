...
export class KanjiEnrichmentService {
  ...
  static async enrichKanjiData(entry: KanjiEntry): Promise<KanjiEntry> {
    try {
      ...
      if (kanjiCharacters.length > 0) {
        ...
        const allRadicals = kanjiDetails.flatMap(details => details.radicals).filter(Boolean);
        ...
        enrichedEntry.kanjiComponents = kanjiCharacters.map((char, index) => {
          ...
          return {
            character: char,
            radicals: kanjiDetails[index]?.radicals || radicalAnalysis.map(r => `${r.radical} (${r.meaning})`),
        ...
        const totalStrokes = kanjiDetails.reduce((sum, details) => sum + (details.strokeCount || 0), 0);
        if (totalStrokes > 0) {
   ...
   */
  private static getCommonRadicals(): { [key: string]: { name: string; meaning: string } } {
    return {
   ...
   */
  static analyzeRadicals(kanjiChar: string): Array<{ radical: string; name: string; meaning: string }> {
