...
export function useKanjis() {
  ...
  const addKanjiFromCharacter = async (
    character: string
    ...
    try {
      ...
      const bestMatch =
        jishoResults.find((entry) =>
          entry.japanese.some(
            (jp) => jp.word === character || jp.reading === character
          )
        ) || jishoResults[0];
      ...
      const allReadings = bestMatch.japanese
        .map((j) => j.reading)
        .filter(Boolean) as string[];
      const categorizedReadings =
        KanjiEnrichmentService.categorizeReadings(allReadings);
  ...
  const addKanjiManually = async (
    kanjiData: Omit<KanjiEntry, "id" | "dateAdded" | "lastModified">
  ): Promise<KanjiEntry | null> => {
    try {
      setError(null);

      // Vérifier si le kanji existe déjà
      const existing = await KanjiStorageService.getKanjiByCharacter(
        kanjiData.kanji
      );
      if (existing) {
        setError("Ce kanji est déjà dans votre collection");
